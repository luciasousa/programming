...
public class LinkedList {
  ...
  public void addLast(String e) {
    NodeString newest = new NodeString(e);
    if (isEmpty())
      first = newest;
    else
      last.next = newest;
  ...
  private boolean contains(NodeString n, E e) {
  ...
  private void print(NodeString n) {
    if (n != null) {
  ...
  public String listaPeq(File f){
	  
	  String line;
	  
	  if(f.hasNextLine() && line.length() <= 20){
		  
		  return line;
	  }
  }
  
  public String listaMed(File f){
	  
	  String line;
	  
	  if(f.hasNextLine() && line.length() > 20 && line.length() < 40){
		  
		  return line;
	  }
  }
  
  public String listaGra(File f){
	  
	  String line;
	  
	  if(f.hasNextLine() && line.length() >= 40){
