...
public class Sorting {
  ...
  public static long assignmentCount = 0L; //para contar atribuições de valor a elementos
  public static long comparisonCount = 0L; //para contar comparações entre elementos do array
    ...
    for (int i=start; i<end-1; i++) { // For each element (except the last):
      for (int j=i+1; j<end; j++) {   // Scan every following element
        if (a[j] < a[i]) {            // compare them
          swap(a, i, j); 
          comparisonCount++;             // if necessary, swap them
        }
    ...
    while (start < end-1) {
      ...
      for (int i=start; i<end-1; i++) {
        if (a[i] > a[i+1]) {
          swap(a, i, i+1);
          last = i;
          comparisonCount++;   // store index of the last swap
        }
  ...
  public static void insertionSort(int[] a, int start, int end) {
    /**Duas partes no vetor, uma odenada que diminui e outra nao ordenada que aumenta
     * ordena atraves da inserçao no segmento ordenado
    **/ 
  ...
  public static void mergeSort(int[] a, int start, int end) {
	/**Metodo recursivo, dividir vetor em 2 ordenar cada vetor 
	 * recursivamente, combinar os dois vetores formando uma lista ordenada
	 **/  
    assert validSubarray(a, start, end);
	if(end - start > 1) {
  ...
  public static void mergeSubarrays(int[] a, int start,int middle, int end) { 
    ...
    int[] b = new int[end - start]; //array auxiliar
    int i1 = start;
    int i2 = middle;
	int j = 0;
  ...
  public static <E extends Comparable<E>> void mergeSort(String[] a, int start, int end) {
	...
	static void mergeSubarrays(String[] a, int start, int middle, int end) {
	
	String[] b = new String[end-start]; // auxiliary array
	int i1 = start;
	int i2 = middle;
	int j = 0;
